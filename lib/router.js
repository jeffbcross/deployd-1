...
Router.prototype.route = function (req, res, next) {
  ...
  console.log('resources', resources, url);
  if (req._routed) {
    return;
  }
  ...
  async.series([function(fn) {
    ...
    async.forEach(router.resources, function(resource, fn) {
      ...
      if(resource.handleSession) {
        var ctx = new Context(resource, req, res, server, next);
  ...
  function nextResource() {
    ...
    handler.run(function () {
      ...
      process.nextTick(function () {
        ...
        if (resource) {
          console.log('handler.run, process.nextTick');
          debug('routing %s to %s', req.url, resource.path);
          ctx = new Context(resource, req, res, server, next);
          ...
          if(resource.external && resource.external[furl]) {
            ...
            resource.handle(ctx, req, res, next || nextResource);
          }
        } else {
          console.log('problem');
          debug('404 %s', req.url);
