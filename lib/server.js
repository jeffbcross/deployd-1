...
Server.prototype.handleRequest = function handleRequest (req, res, next) {
  ...
  setupReqRes(req, res, function(err) {
...
Server.prototype.route = function route (req, res, next) {
  console.log('server.route()');
  var server = this;

  config.loadConfig('./', server, function(err, resourcesInstances) {
    console.log('loadConfig', err);
    if (err) throw err;
    var router = new Router(resourcesInstances, server);
    server.router = router;

    server.resources = resourcesInstances;
    console.log('about to router.route');
